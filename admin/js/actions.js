...
$(".unverified_btn").click(function () {
  var user_id_v = $(this).data("userId");
  var button = this;
  $(button).attr("disabled", true);

  $.ajax({
    url: "php/admin_ajax.php?unverified_user",
    method: "post",
    dataType: "json",
    data: { user_id: user_id_v },
    success: function (response) {
      ...
      if (response.status) {
        $(button).hide();
        $(button).siblings(".verified_btn").show();
        $(button).siblings(".verified_btn").attr("disabled", false);
      } else {
        $(button).attr("disabled", false);
        alert("something is wrong,try again after some time");
      }
...
$(".verified_btn").click(function () {
  ...
  $.ajax({
    url: "php/admin_ajax.php?verified_user",
    method: "post",
    dataType: "json",
    data: { user_id: user_id_v },
    success: function (response) {
      ...
      if (response.status) {
        $(button).hide();
        $(button).siblings(".unverified_btn").show();
        $(button).siblings(".unverified_btn").attr("disabled", false);
      } else {
        $(button).attr("disabled", false);
        alert("something is wrong,try again after some time");
      }
