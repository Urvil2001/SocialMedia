...
function ReqVeriCount(){
    global $db;
    $query="SELECT count(*) as row FROM verification";
    $run = mysqli_query($db,$query);
    return mysqli_fetch_assoc($run)['row'];
}
...
function Verified(){
    global $db;
    $query="SELECT count(*) as row FROM users where verification='verified'";
    $run = mysqli_query($db,$query);
    return mysqli_fetch_assoc($run)['row'];
}
...
function unverified_user($user_id){
    global $db;
    $query="UPDATE verification,users SET verification.verification='Unverified',users.verification='unverified' WHERE verification.email=users.email AND verification.id=$user_id";
    return mysqli_query($db,$query);
}

function verified_user($user_id){
    global $db;
    $query="UPDATE verification,users SET verification.verification='Verified',users.verification='verified' WHERE verification.email=users.email AND verification.id=$user_id";
