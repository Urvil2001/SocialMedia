...
?>
    <div class="container col-md-9 col-sm-11 rounded-0">
        <div class="col-12 rounded p-4 mt-4 d-md-flex gap-5">
            ...
            <?php if($profile['verification']=='verified')
            { ?>
                <span style="font-size: xx-large;color: blue;"><?=$profile['first_name']?> <?=$profile['last_name']?></span>
            <?php }else{
                ?>
...
foreach($profile_post as $post){
    ...
    ?>
            <img src="assets/images/posts/<?=$post['post_img']?>" data-bs-toggle="modal" data-bs-target="#postview<?=$post['id']?>" width="300px" height="250px" class="rounded" />

            
       	    <!--<video>
		<source src ="assets/images/posts/<?=$post['post_img']?>" data-bs-toggle="modal" data-bs-target="#postview<?=$post['id']?>" width="300px" class="rounded" />
	    </video>--!>
        ...
        <div class="modal-dialog modal-xl modal-dialog-centered">
			...
			<!--<video><source src="assets/images/posts/<?=$post['post_img']?>" style="max-height:90vh" class="w-100 rounded-start"></video>--!>
                    </div>
